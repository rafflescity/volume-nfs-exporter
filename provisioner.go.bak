...
func (p *volumeNfsProvisioner) Provision(options controller.ProvisionOptions) (*v1.PersistentVolume, error) {
	...
	nfsExportMethod := options.StorageClass.Parameters[ "nfsExportMethod" ]
	klog.Infof( "Data backend SC is \"%s\"", dataScName )
	klog.Infof( "NFS Export Method is \"%s\"", nfsExportMethod )
	...
	RunExtCmd( "create-nfs-sts.sh", nfsPvcNs, nfsStsName, nfsPvcName, nfsPvName, dataPvcName, dataPvName, nfsExportMethod )
